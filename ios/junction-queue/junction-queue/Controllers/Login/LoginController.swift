...
class LoginController: UIViewController {
			...
			if let user = user {
				...
				docRef.getDocument { (document, error) in
					...
					if let userObject = User(dictionary: document.data()) {
						AuthenticationService.authenticate(userUid: user.uid, name: userObject.name, email: email)
    ...
    func onRegister() {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (user, error) in
            ...
            let newUser = User(name: name, email: email, cardNumber: "0000111122223333")
    ...
    override func viewDidLoad() {
        ...
        // todo: remove
        emailTextField.text = "latnikov@gmail.com"
